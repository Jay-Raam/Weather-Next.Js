...
const FlickeringGrid: React.FC<FlickeringGridProps> = ({
  ...
  const memoizedColor = useMemo(() => {
    const toRGBA = (color: string) => {
      if (typeof window === "undefined") return "rgba(0, 0, 0,"; // Fallback for server-side
      const canvas = document.createElement("canvas");
      canvas.width = 1;
      canvas.height = 1;
      const ctx = canvas.getContext("2d");
      if (!ctx) return "rgba(255, 0, 0,"; // Fallback in case of error
      ctx.fillStyle = color;
  ...
  return (
    <canvas
      ...
      width={width} // Use specified width or let it be auto
      height={height} // Use specified height or let it be auto
    />
  );
};
